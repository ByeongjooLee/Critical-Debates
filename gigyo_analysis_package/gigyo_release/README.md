...
> ⚠️ **BHR 절대 해석 주의** — 위 C1의 "수렴"·"유보/단언" 라벨은 LOO·부트스트랩(텍스트 교란)엔 견고하나 **완화 마커 코딩에 의존**한다. KWIC 검토 결과 완화 1위 `ㄹ 수 있다`는 이 비평 레지스터에서 대부분 제시적·관찰적 용법(비완화)이고, 강조 1위 `물론`은 양보 표지였다. 완충 `ㄹ 수 있다`와 강조 `물론`을 모두 용법별로 재분류하면 BHR은 **박용철 0.44·임화 1.07·김기림 0.71**이 되어(임화만 1을 넘고, 상대 서열 임>김>박은 불변), "셋 다 유보적 수렴"은 성립하지 않는다. 코딩-불변 결론(상대 서열·정성 기술)만 신뢰한다. 재집계·민감도: `run_bhr_sense.py`·`run_bhr_formrule.py`(완충 수있다)·`run_bhr_mullon.py`(강조 물론)·`run_stance_profile.py`. (강조 `물론` 재분류는 현재 단일 코더 판정으로, 제2 코더 검증은 후속 과제.) **수작업 교차검증**: 40개 표본을 사람이 직접 코딩 → 형태규칙과 40/40 일치, 그리고 제2 코더(현대문학 박사수료)의 **블라인드 독립 코딩과도 40/40 일치(Cohen's κ=1.00)**. 단 비완화 95% 편중으로 κ가 불안정하고 두 코더가 동일 분과 배경이라는 한계 — `validation/` 참고.
